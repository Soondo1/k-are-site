...
function SanityLive(props) {
  const {
    ...
    ), setLongPollingInterval(!1)) : event.type === "message" ? serverActions.revalidateSyncTags(event.tags) : event.type === "restart" || event.type === "reconnect" ? router.refresh() : event.type === "goaway" && (onGoAway(event, intervalOnGoAway), setLongPollingInterval(intervalOnGoAway));
  });
  React.useEffect(() => {
